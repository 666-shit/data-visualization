...
pie = Pie(init_opts=opts.InitOpts(width='1350px', height='750px', renderer='svg', bg_color='white'))
pie.set_colors(color_series)
# 添加数据 设置饼图的半径 展示成玫瑰图
pie.add(
    "", [list(z) for z in zip(pname, pdata)],
    radius=["30%", "135%"],
    center=["50%", "65%"],
    rosetype="area"
)
pie.set_global_opts(
    title_opts=opts.TitleOpts(title='疫情数据玫瑰图'),
...
pie.render(r"..\chart\pie\全国疫情玫瑰图.html")
